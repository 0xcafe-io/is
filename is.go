...
var reIsSourceFile = regexp.MustCompile(`is\.go$`)
...
// JSONEqual asserts that a and b are equal JSONs (ignoring formatting and key order).
func (is *I) JSONEqual(a, b string) {
	var expectedVal any
	if err := json.Unmarshal([]byte(a), &expectedVal); err != nil {
		is.log("invalid JSON: %s: %s", a, err.Error())
		return
	}
	...
	var actualVal any
	if err := json.Unmarshal([]byte(b), &actualVal); err != nil {
		is.log("invalid JSON: %s: %s", a, err.Error())
	}
	is.Equal(expectedVal, actualVal)
}
